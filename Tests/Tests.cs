    ...
    static class Main {
        public static bool RunTests(string projectRoot, 
            ...
            if (runWin32Tests) {
                ...
                if (!Tests_Symlinks.Test_Symlinks1_Final(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks1_Target(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks2_Final()) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks2_Target()) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks3_Final(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks3_Target(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks4_Final(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks4_Target(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks5_Final(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks5_Target(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks6_Final(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks6_Target(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks7_Final(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks7_Target(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks8_Final(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks8_Target(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks9()) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks10()) returnVal = false;
                if (!Tests_Symlinks.Test_Symlinks11(rootTestFolder)) returnVal = false;
                if (!Tests_Symlinks.Test_SymlinkThrows1(rootTestFolder)) returnVal = false;
                ...
                if (!Tests_Symlinks.Test_SymlinkThrows4()) returnVal = false;
                if (!Tests_Symlinks.Test_SymlinkThrows5()) returnVal = false;
                if (!Tests_Symlinks.Test_SymlinkThrows6()) returnVal = false;
                if (!Tests_Symlinks.Test_SymlinkThrows7()) returnVal = false;
                if (!Tests_Symlinks.Test_SymlinkThrows8(rootTestFolder)) returnVal = false;
